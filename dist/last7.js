...
const daysBackDate = new Date(currentDate);
...
const daysBackYear = daysBackDate.getFullYear();
const daysBackMonth = String(daysBackDate.getMonth() + 1).padStart(2, "0");
const daysBackDay = String(daysBackDate.getDate()).padStart(2, "0");
...
const getGamesData = () => {
	fetch(
		`https://api.rawg.io/api/games?dates=${daysBackYear}-${daysBackMonth}-${daysBackDay},${currentYear}-${currentMonth}-${currentDay}&ordering=-released&key=${apiKey}&page=${currentPage}&page_size=${gamesPerPage}`
	)
		.then((res) => res.json())
		.then((data) => {
			games = [];
