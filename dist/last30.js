...
const monthBackDate = new Date(currentDate);
...
const monthBackYear = monthBackDate.getFullYear();
const monthBackMonth = String(monthBackDate.getMonth() + 1).padStart(2, "0");
const monthBackDay = String(monthBackDate.getDate()).padStart(2, "0");
...
const getGamesData = () => {
	fetch(
		`https://api.rawg.io/api/games?dates=${monthBackYear}-${monthBackMonth}-${monthBackDay},${currentYear}-${currentMonth}-${currentDay}&ordering=-released&key=${apiKey}&page=${currentPage}&page_size=${gamesPerPage}`
	)
		.then((res) => res.json())
		.then((data) => {
			games = [];
